    ...
    public class DoCookie
    {
        ...
        public static void WriteCookie(string CookieName, string Value)
        {
            try
            {
                ...
                {
                    HttpContext.Current.Response.Cookies.Append(CookieName, DoEncrypt.Encrypt(DoWebRequest.UrlEncode(Value)));
                }
            }
            catch (Exception)
            {
        ...
        public static void WriteCookie(string CookieName, string Value, CookieOptions Options)
        {
            try
            {
                if (HttpContext.Current.Request.Cookies != null)
                {
                    ...
                    HttpContext.Current.Response.Cookies.Append(CookieName, DoEncrypt.Encrypt(DoWebRequest.UrlEncode(Value)), Options);
                }
            }
            catch (Exception)
            {
        ...
        public static string GetCookie(string CookieName)
        {
            ...
            {
                return DoEncrypt.Decrypt(DoWebRequest.UrlDecode(HttpContext.Current.Request.Cookies[CookieName]));
            }
            return null;
        }

        public static string GetwebCookie(string CookieName)
        {
            if (HttpContext.Current.Request.Cookies != null && HttpContext.Current.Request.Cookies[CookieName] != null)
            {
