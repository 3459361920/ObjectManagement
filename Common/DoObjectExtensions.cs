...
namespace Wathet.Common
{
    ...
    public static class DoObjectExtensions
    {
        ...
        public static object ToObjectFromJson(this string str)
        {
            ...
            {
                Console.WriteLine(ex.ToSimple("Common.ToObjectFromJson报错，源字符串：【" + str + "】"));
                return null;
            }
        }
        ...
        /// </summary>
        public static string GetValue<TValue>(this Dictionary<string, TValue> dict, string key, string defaultValue = default(string), bool isToLower = true)
        {
            ...
            if (dict.ContainsKey(key))
            {
                ...
                if (body != null)
                {
                    if (body is Wathet.Common.Entity.vvv)
                    {
                        var vv = body as Wathet.Common.Entity.vvv;
                        if (vv.time >= DateTime.Now)
                        {
                            return vv.value;
                        }
                    }
