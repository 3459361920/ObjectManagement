    ...
    public static class DoIOFile
    {
        ...
        public static string Read_LastRow(string sourceFile, long splitFileSize = 1024 * 1024 * 1, char separate = '\n')
        {
            ...
            if (historyFile.Exists)
            {
                ...
                var isLeave = false;
                var contentHistory = Wathet.Common.DoIOFile.Read_BigFile_1M(historyFile.FullName, ref beginPosition, ref isLeave, tempLength * 10);
        ...
        /// </summary>
        /// <param name="filePath">文件路径(支持相对路径或绝对路径)</param>
        /// <param name="stream">要写入文件的流对象</param>        
        /// <param name="IsAppend">是否续写文件,false的话会覆盖文件原内容</param>
        /// <param name="Encode">编码方式,默认UTF-8</param>
        ...
        public static int Get_FileLineCount(string filePath)
        {
            ...
            {
                //每次1M文件
                string content = Wathet.Common.DoIOFile.Read_BigFile_1M(filePath, ref readStart, ref contentLeave, 1024 * 1024 * 1);
        ...
        public static byte[] ToBytes(Stream s)
        {
            ...
            byte[] imgBytesIn = br.ReadBytes((int)ms.Length);
            return imgBytesIn;
        }
        #endregion
    }
