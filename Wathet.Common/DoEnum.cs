...
namespace Wathet.Common
{
    public static class DoEnum
    {
        ...
        public static string GetNameByNumber<T>(int number)
        {
            var obj = Enum.Parse(typeof(T), number.ToString()) as Enum;
            var name = obj.GetEnumName();
            return obj != null && obj.GetType().IsEnum && name != null  ? name: string.Empty;
        }
        ...
        public static int GetCodeByDes<T>(string desc)
        {
            foreach (var item in Enum.GetNames(typeof(T)))
            {
                var obj = Enum.Parse(typeof(T), item) as Enum;
                if (obj.GetDesc() == desc)
        ...
        /// <returns></returns>
        public static JsonRSimple GetJsonrSimple<T>(int number)
        {
            var obj = Enum.Parse(typeof(T), number.ToString()) as Enum;
            var dic = obj.GetDescAll();
            if (obj != null && obj.GetType().IsEnum && dic != null && dic.ContainsKey(number))
            {
                return new JsonRSimple(number, dic[number]);
            }
            return new JsonRSimple();
        }
