...
namespace Wathet.Common
{
    public class DoExcel
    {
        ...
        public static bool Create<T>(List<T> list, string filePath, bool IsOverWrite = true, bool IsAppendTitle = true, string[] Headers = null, string fileName = "", bool isCsv = false)
        {
            if (isCsv)
            {
                return CreateCSV(list.ToDataTable<T>(), filePath, IsAppendTitle, IsOverWrite, Headers, fileName);
            }
            else
            {
        ...
        public static bool Create(DataTable dt, string filePath, bool IsOverWrite = true, bool IsAppendTitle = true, string[] Headers = null, string fileName = "", bool isCsv = false)
        {
            if (isCsv)
            {
                return CreateCSV(dt, filePath, IsAppendTitle, IsOverWrite, Headers, fileName);
            }
            try
            {
                dt = dt ?? new DataTable();
        ...
        /// 使用List<T>生成CSV文件
        /// </summary>
        public static bool CreateCSV<T>(List<T> list, string filePath, bool IsAppendTitle = true, bool IsOverWrite = true, string[] Headers = null, string fileName = "")
        {
            return CreateCSV(list.ToDataTable<T>(), filePath, IsAppendTitle, IsOverWrite, Headers, fileName);
        }
        #endregion
        ...
        /// 用datatable生成CSV文件
        /// </summary>
        public static bool CreateCSV(DataTable dt, string filePath, bool IsAppendTitle = true, bool IsOverWrite = true, string[] Headers = null, string fileName = "")
        {
            try
            {
                dt = dt ?? new DataTable();
                ...
                if (IsAppendTitle)
                {
                    context += GetCsvRow(Headers != null ? Headers : dt.GetColumnNames().ToArray()) + "\n"; ;
        ...
        public static string DownloadFile(string url, string directory, string fileNamenew)
        {
            try
            {
                directory = directory.Replace("\\", "/").Replace(@"\", "/");
                var fileName = Path.GetFileName(url);
                fileNamenew = fileNamenew + DateTime.Now.ToString("yyyyMMdd");
                if (!Directory.Exists(fileNamenew))   //判断文件路径是否存在，不存在则创建文件夹
                {
                    Directory.CreateDirectory(fileNamenew);//创建路径下的文件夹
                }
                WebClient client = new WebClient();
                fileNamenew = fileNamenew + "/";
                client.DownloadFile(url, fileNamenew + fileName);
                return fileNamenew + fileName;
            }
            catch (Exception ex)
            {
                return ex.Message;
        ...
        /// <returns></returns>
        public static DataTable ByteToDataTable(byte[] bytes)
        {
            Stream stream = new MemoryStream(bytes);
            var workbook = new HSSFWorkbook(stream);
            if (workbook != null)
            {
                return GetDtBySheet(workbook.GetSheetAt(0), true);
            }
            return null;
        }
