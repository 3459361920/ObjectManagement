...
namespace Wathet.Common
{
    ...
    public class DoFtps
    {
        ...
        #region //打开连接(设置超时时长)
        public bool OpenClientNew(int time_out = 0)
        {
            ...
            try
            {
                ...
                if (!client.IsConnected) //连接服务
                {
                    if (time_out != 0)
                    {
                        client.ConnectTimeout = time_out;
                    }
                    client.Connect();
                }
                return true;
        ...
        {
            return UploadNew(remotePath, localPath, 0);
        }

        #endregion

        #region 上传单个文件（设置响应时长）
        /// <summary>
        /// 上传单个文件
        /// </summary>
        /// <param name="remotePath">带文件名的服务器路径</param>
        /// <param name="localPath">本地文件</param>
        /// <returns></returns>
        public bool UploadNew(string remotePath, string localPath, int time_out = 0)
        {
            try
            {
                ...
                if (file.Exists)
                {
                    var is_open = OpenClientNew(time_out); //初始化连接
                    if (is_open)
                    {
                        //检查文件夹是否存在,不存在则创建
        ...
        public bool IsExists(string RemotePath)
        {
            ...
            {
                Console.WriteLine(ex.ToSimple("获取文件是否存在"));
                return false;
            }
            finally
            {
